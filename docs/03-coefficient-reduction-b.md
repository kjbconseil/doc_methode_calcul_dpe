...
| Local non chauffé non accessible (moyennement, espaces sans accès...) | Forfaitairement **b = 0.95** |
| Paroi donnant sur un bâtiment ou espace autre que d'habitation (occupation discontinue) | Considerée comme déperditive avec **b = 0.2** |

### Cas des circulations communes en collectif

Pour les circulations communes au niveau d'un appartement en bâtiment collectif d'habitation, le calcul de b se fait en considérant les parois situées au même niveau que le lot traité :
...
**Règles de calcul** :
- `Aiu` est la surface des parois du local non chauffé donnant sur les **locaux chauffées** (cote intérieur)
- `Aue` est la surface des parois du local non chauffé donnant sur **l'extérieur** ou en contact avec le sol (parois enterrées, terre-plein)
...
Les espaces tampons solarisés (vérandas, loggias fermées et non chauffées) bénéficient d'apports solaires qui y génèrent des températures supérieures a celles attendues dans les espaces non solarisés.
...
- L'orientation de la véranda pour en connaitre est celle de la facade principale (avec la plus grande surface de vitrages verticaux). S'il existe plusieurs facades d'orientation prédominante, c'est-a-dire qu'au moins deux facades d'orientation predominant de façon égale les surfaces vitrées les plus importantes, b_vu est la moyenne des b_vu sur ces orientations
